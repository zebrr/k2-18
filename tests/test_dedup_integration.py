...
def create_test_graph_with_semantic_duplicates():
    ...
    # Добавляем рёбра
    edges.extend([
        {'source': 'chunk_vars_1', 'target': 'concept_variable', 'type': 'MENTIONS', 'weight': 1.0},
        {'source': 'chunk_vars_2', 'target': 'concept_variable', 'type': 'MENTIONS', 'weight': 1.0},
        {'source': 'chunk_vars_3', 'target': 'concept_variable', 'type': 'MENTIONS', 'weight': 1.0},
        {'source': 'chunk_vars_1', 'target': 'chunk_for_1', 'type': 'PREREQUISITE', 'weight': 0.8},
        {'source': 'chunk_vars_2', 'target': 'chunk_unique_1', 'type': 'ELABORATES', 'weight': 0.6},
        {'source': 'chunk_for_1', 'target': 'chunk_unique_2', 'type': 'HINT_FORWARD', 'weight': 0.7},
        {'source': 'chunk_vars_3', 'target': 'assessment_1', 'type': 'TESTS', 'weight': 0.9},
        {'source': 'chunk_for_2', 'target': 'assessment_2', 'type': 'TESTS', 'weight': 0.85},
    ])
...
class TestDedupIntegrationReal(unittest.TestCase):
    ...
    def test_semantic_duplicates_two_thresholds(self):
        ...
        result_count_low = len([n for n in result_low_threshold['nodes'] if n['type'] in ['Chunk', 'Assessment']])
        ...
        for entry in dedup_entries_low[:5]:  # Показываем первые 5
            print(f"    {entry['duplicate_id']} -> {entry['master_id']} (similarity: {entry['similarity']})")
        ...
        result_count_high = len([n for n in result_high_threshold['nodes'] if n['type'] in ['Chunk', 'Assessment']])
        ...
        if result_count_high == original_count:
            print("  No duplicates found - threshold 0.97 is very high for semantic similarity")
        ...
        print(f"Difference: {len(dedup_entries_low) - len(dedup_entries_high)} more duplicates with lower threshold")
    ...
    def test_edge_cases_with_real_api(self):
        ...
        identical_entries = [e for e in dedup_entries 
    ...
    def test_rate_limiting_handling(self):
        ...
        for i in range(50):  # Уменьшил до 50 для скорости
            nodes.append({
                'id': f'chunk_load_{i}',
                'type': 'Chunk',
                'text': f'This is test chunk number {i} with unique content to test rate limiting. ' * 10,
                'local_start': i * 100
            })
    ...
    def test_invalid_graph_structure(self):
        ...
        invalid_graph = {
            'nodes': [
                {'id': 'test', 'type': 'Chunk', 'text': 'Test', 'local_start': 0}
            ]
            # 'edges' отсутствует!
        }
