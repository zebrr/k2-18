...
- **Latest checkpoint**: `/data/out/LearningChunkGraph_raw_latest.json` - latest fully integrated successful slice state
- **Slice checkpoints**: `/data/out/checkpoints/LearningChunkGraph_raw_after_slice_XXX.json` - immutable checkpoint after each successful slice
...
### Successful Slice Checkpoints
- After every fully integrated slice, the utility writes a recoverable graph artifact
- Checkpoint is saved only after JSON parsing, ID post-processing, graph merge, intermediate validation, and `previous_response_id` update succeed
- `/data/out/LearningChunkGraph_raw_latest.json` is atomically replaced with the latest successful state
- `/data/out/checkpoints/LearningChunkGraph_raw_after_slice_XXX.json` preserves a per-slice checkpoint history
- Checkpoint metadata includes `complete=false`, last completed slice id/order/token range, processed slice count, total slice count, `previous_response_id`, and stats
- Final `/data/out/LearningChunkGraph_raw.json` uses the same output structure and marks `complete=true`

### Resume From Latest Checkpoint
- `resume_from_latest=true` restores `/data/out/LearningChunkGraph_raw_latest.json` before processing
- Restored state includes nodes, edges, rebuilt node ID index, `previous_response_id`, API usage, quality issue counters, and processing stats
- The utility validates checkpoint compatibility with current staging before continuing
- Compatibility checks include total slice count, last completed slice id/order/token range, and slug
- Already completed slices are skipped; processing continues with slices whose `order` is greater than the checkpoint's last completed order
- If the saved `previous_response_id` is no longer accepted by the API, processing stops with `EXIT_RUNTIME_ERROR`; it does not silently continue without response-chain context

## Core Algorithm

1. **Load ConceptDictionary** - must exist from itext2kg_concepts.py
2. **Load slices** from staging in lexicographic order
   - If `resume_from_latest=true`, load latest checkpoint, validate staging compatibility, and skip completed slices
3. **Sequential processing** with previous_response_id preservation:
   ...
   - Save atomic latest and per-slice checkpoints after successful integration
4. **Critical error handling**:
   - If any slice fails → exit with RUNTIME_ERROR
   - Previously completed slices remain available in latest/per-slice checkpoints
   - Save temporary dumps for additional recovery diagnostics
...
Section `[itext2kg_graph]` in config.toml:
...
- **resume_from_latest** (bool, default=false) - resume from `/data/out/LearningChunkGraph_raw_latest.json`
