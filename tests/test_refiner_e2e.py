...
class TestGraphLoadValidate(unittest.TestCase):
    ...
    def setUp(self):
        self.sample_graph = {
            "nodes": [
                {"id": "node1", "type": "Chunk", "text": "Test chunk 1", "local_start": 0},
                {"id": "node2", "type": "Chunk", "text": "Test chunk 2", "local_start": 100},
                {"id": "node3", "type": "Concept", "text": "Test concept", "local_start": 300},
                {"id": "node4", "type": "Assessment", "text": "Test question", "local_start": 200}
            ],
            "edges": [
...
class TestEmbeddingsIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.nodes = [
            {"id": "node1", "type": "Chunk", "text": "Python variables", "local_start": 0},
            {"id": "node2", "type": "Chunk", "text": "Python functions", "local_start": 100},
            {"id": "node3", "type": "Assessment", "text": "Test question", "local_start": 200}
        ]
    ...
    def test_get_node_embeddings_success(self, mock_get_embeddings):
        ...
        mock_get_embeddings.assert_called_once_with(
            ["Python variables", "Python functions", "Test question"],
            self.config
        )
...
class TestFAISSIntegration(unittest.TestCase):
    ...
    def test_build_faiss_index_and_search(self):
        ...
        candidates = generate_candidate_pairs(
            self.nodes, embeddings_dict, index, node_ids,
            edges_index, self.config, self.logger
        )
    ...
    def test_faiss_with_different_metrics(self):
        ...
        embeddings_dict = {
            f"n{i}": np.random.rand(1536).astype(np.float32) 
            for i in range(1, 5)
        }
...
class TestRefinerFullPipeline(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.graph_data = {
            "nodes": [
                {"id": "n1", "type": "Chunk", "text": "Python variables", "local_start": 0},
                {"id": "n2", "type": "Chunk", "text": "Python functions", "local_start": 1000},
                {"id": "n3", "type": "Chunk", "text": "Python classes", "local_start": 2000},
                {"id": "concept1", "type": "Concept", "text": "Variables", "local_start": 3000}
            ],
            "edges": [
                {"source": "n1", "target": "concept1", "type": "MENTIONS"}
            ]
        }
    ...
    @patch('pathlib.Path.exists')
    def test_pipeline_no_candidates(self, mock_exists, mock_file, mock_config, 
                                   mock_embeddings, mock_openai_client):
    ...
    @patch('builtins.open', new_callable=mock_open)
    @patch('pathlib.Path.exists')
    def test_pipeline_with_analysis(self, mock_exists, mock_file, mock_config,
                                   mock_embeddings, mock_openai_client):
        ...
        self.assertTrue(any("LearningChunkGraph.json" in str(call) 
    ...
    @patch('builtins.open', new_callable=mock_open)
    @patch('pathlib.Path.exists')
    def test_pipeline_with_repair_retry(self, mock_exists, mock_file, mock_config,
                                       mock_embeddings, mock_openai_client):
    ...
    @patch('builtins.open', new_callable=mock_open)
    @patch('pathlib.Path.exists')
    def test_pipeline_api_error(self, mock_exists, mock_file, mock_config,
                               mock_embeddings, mock_openai_client):
...
class TestLoggingIntegration(unittest.TestCase):
    ...
    @patch('refiner.load_config')
    @patch('pathlib.Path.exists')
    def test_json_logging_setup(self, mock_exists, mock_config, 
                               mock_embeddings, mock_openai_client):
        """Тест настройки JSON логирования."""
        from refiner import main, setup_json_logging
        ...
        with patch('builtins.open', mock_open(read_data=json.dumps(graph_data))) as mock_file:
            with patch('logging.FileHandler') as mock_handler:
                ...
                with patch('utils.validation.validate_json'):
                    with patch('utils.validation.validate_graph_invariants'):
                        ...
                        self.assertEqual(logger.level, logging.DEBUG)  # log_level = "debug"
