...
class TestConfigLoading:
    ...
    def test_invalid_toml_syntax(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".toml", delete=False) as f:
            ...
            with pytest.raises(
                ConfigValidationError, match="Failed to parse TOML file"
            ):
                load_config(f.name)
...
class TestSlicerValidation:
    ...
    def test_invalid_max_tokens(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".toml", delete=False) as f:
            ...
            with pytest.raises(
                ConfigValidationError, match="slicer.max_tokens must be positive"
            ):
                load_config(f.name)
    ...
    def test_overlap_soft_boundary_validation(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".toml", delete=False) as f:
            ...
            with pytest.raises(
                ConfigValidationError, match="cannot exceed overlap\\*0.8"
            ):
                load_config(f.name)
    ...
    def test_empty_api_key(self):
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(
                ...
                with pytest.raises(
                    ConfigValidationError, match="api_key not configured"
                ):
                    load_config(f.name)
        finally:
