...
from src.utils.llm_client import TPMBucket, OpenAIClient, ResponseUsage, IncompleteResponseError
...
class TestOpenAIClient:
    ...
    @patch('src.utils.llm_client.tiktoken.get_encoding')
    @patch('src.utils.llm_client.OpenAI')
    def test_successful_response(self, mock_openai_class, mock_get_encoding, mock_probe, test_config):
        ...
        # Mock headers
        initial_headers = {'x-ratelimit-remaining-tokens': '100000', 'x-ratelimit-reset-tokens': '60000ms'}
        status_headers = {'x-ratelimit-remaining-tokens': '95000', 'x-ratelimit-reset-tokens': '1736622600'}
        ...
        assert mock_client_instance.responses.with_raw_response.retrieve.call_count == len(retrieve_responses)
    ...
    def test_reasoning_model_response(self, mock_openai_class, mock_get_encoding, reasoning_config):
        ...
        # Headers
        headers = {'x-ratelimit-remaining-tokens': '58000', 'x-ratelimit-reset-tokens': '30000ms'}
    ...
    @patch('src.utils.llm_client.tiktoken.get_encoding')
    @patch('src.utils.llm_client.OpenAI')
    def test_previous_response_id(self, mock_openai_class, mock_get_encoding, mock_probe, test_config):
        ...
        second_call_args = mock_client_instance.responses.with_raw_response.create.call_args_list[1][1]
    ...
    @patch('src.utils.llm_client.tiktoken.get_encoding')
    @patch('src.utils.llm_client.OpenAI')
    def test_rate_limit_retry_with_headers(self, mock_openai_class, mock_get_encoding, mock_sleep, mock_probe, test_config):
    ...
    def test_repair_response(self, mock_openai_class, mock_get_encoding, mock_probe, test_config):
        ...
        repair_call_args = mock_client_instance.responses.with_raw_response.create.call_args_list[1][1]
    ...
    @patch('src.utils.llm_client.tiktoken.get_encoding')
    @patch('src.utils.llm_client.OpenAI')
    def test_incomplete_response_handling(self, mock_openai_class, mock_get_encoding, mock_sleep, mock_probe, test_config):
    ...
    @patch('src.utils.llm_client.tiktoken.get_encoding')
    @patch('src.utils.llm_client.OpenAI')
    def test_failed_status_handling(self, mock_openai_class, mock_get_encoding, mock_sleep, mock_probe, test_config):
        ...
        failed_response = Mock(
            id="resp_001",
            status="failed",
            error=Mock(message="Internal error")
        )
...
class TestLogging:
    ...
    @patch('src.utils.llm_client.tiktoken.get_encoding')
    @patch('src.utils.llm_client.OpenAI')
    def test_multiple_retries_with_logging(self, mock_openai_class, mock_get_encoding, mock_sleep, mock_probe, caplog):
        ...
        # Настраиваем последовательность
        mock_client_instance.responses.with_raw_response.create.side_effect = [
            errors[0], errors[1], errors[2], mock_raw_initial
        ]
    ...
    @patch('src.utils.llm_client.tiktoken.get_encoding')
    @patch('src.utils.llm_client.OpenAI')
    def test_max_retries_exceeded_logging(self, mock_openai_class, mock_get_encoding, test_config, caplog):
...
class TestTPMBucketEnhanced:
    ...
    def test_update_from_headers_reimbursement(self, caplog):
        ...
        headers = {
            'x-ratelimit-remaining-tokens': '100000'  # Больше чем было
        }
