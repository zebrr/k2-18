...
from src.utils.exit_codes import (EXIT_IO_ERROR, EXIT_RUNTIME_ERROR,
                                  EXIT_SUCCESS)
...
def setup_test_env(tmp_path, mock_config):
    ...
    prompt_file.write_text(
        "Test prompt {concept_dictionary_schema} {learning_chunk_graph_schema}"
    )
    ...
    (tmp_path / "schemas" / "LearningChunkGraph.schema.json").write_text(
...
class TestErrorHandlingMethods:
    ...
    def test_save_bad_response(self, setup_test_env, mock_config):
        ...
        with patch("src.itext2kg.PROMPTS_DIR", setup_test_env / "prompts"), patch(
            "src.itext2kg.SCHEMAS_DIR", setup_test_env / "schemas"
...
class TestRepairLogic:
    ...
    def test_successful_repair(self, setup_test_env, mock_config):
        ...
        with patch("src.itext2kg.PROMPTS_DIR", setup_test_env / "prompts"), patch(
            ...
            "src.itext2kg.STAGING_DIR", setup_test_env / "data" / "staging"
        ), patch(
            "src.itext2kg.OUTPUT_DIR", setup_test_env / "data" / "out"
        ), patch(
            "src.itext2kg.LOGS_DIR", setup_test_env / "logs"
        ):
...
class TestGracefulDegradation:
    ...
    def test_partial_failure_handling(self, setup_test_env, mock_config):
        ...
        with patch("src.itext2kg.PROMPTS_DIR", setup_test_env / "prompts"), patch(
            ...
            for i in range(3):
                slice_file = (
                    setup_test_env / "data" / "staging" / f"test_{i}.slice.json"
                )
                slice_data = {
                    "id": f"slice_{i:03d}",
                    "order": i,
    ...
    def test_validation_error_in_finalize(self, setup_test_env, mock_config):
        ...
        with patch("src.itext2kg.PROMPTS_DIR", setup_test_env / "prompts"), patch(
            ...
            with patch("src.itext2kg.validate_json") as mock_validate:
                ...
                stats_files = list(
                    logs_dir.glob("processing_stats_validation_failed_*.json")
                )
    ...
    def test_io_error_in_finalize(self, setup_test_env, mock_config):
        ...
        with patch("src.itext2kg.PROMPTS_DIR", setup_test_env / "prompts"), patch(
            ...
            try:
                ...
                # Проверяем, что хотя бы какие-то файлы были сохранены в logs
                logs_dir = setup_test_env / "logs"
                # Ищем любые файлы, связанные с io_error
                temp_files = list(logs_dir.glob("*io_error*.json"))
