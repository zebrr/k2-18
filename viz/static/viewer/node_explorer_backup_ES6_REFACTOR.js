...
const NodeExplorer = {
    ...
    // Cluster colors from main visualization
    clusterColors: [
    ...
    metricsInfo: {
        'degree_in': {
            ...
            tooltip: 'Количество узлов, от которых зависит данный материал. Высокое значение указывает на тему, требующую много предварительных знаний.'
        },
        'degree_out': {
            label: 'Исходящая степень',
            tooltip: 'Количество узлов, которые зависят от данного материала. Высокое значение означает, что это базовая тема, открывающая доступ к новым концепциям.'
        },
        'degree_centrality': {
            label: 'Нормализованная связность',
            tooltip: 'Доля всех возможных связей узла относительно размера графа. Показывает, насколько узел интегрирован в общую структуру знаний.'
        },
        'pagerank': {
            label: 'Важность',
            tooltip: 'Определяет значимость узла в структуре знаний. Чем выше значение, тем больше важных концептов ссылается на этот материал. Изучение узлов с высокой важностью критично для полного понимания темы.'
        },
        'betweenness_centrality': {
            label: 'Мост между частями',
            tooltip: 'Показывает, насколько узел является мостом между разными частями учебного материала. Высокое значение означает, что узел соединяет различные темы. Пропуск такого материала может нарушить целостность понимания.'
        },
        'out-closeness': {
            label: 'Исходящая близость',
            tooltip: 'Насколько быстро из данного узла можно достичь других тем. Высокое значение означает хорошую стартовую точку для изучения.'
        },
        'component_id': {
            label: 'Компонента связности',
            tooltip: 'Идентификатор группы взаимосвязанных тем. Узлы с одинаковым ID образуют единый связный блок знаний.'
        },
        'prerequisite_depth': {
            label: 'Уровень зависимостей',
            tooltip: 'Глубина в дереве предварительных требований. 0 = базовые концепты, далее по возрастанию сложности.'
        },
        'learning_effort': {
            label: 'Учебная сложность',
            tooltip: 'Суммарная сложность изучения с учётом всех предварительных тем. Отражает общий объём усилий, необходимых для освоения этого узла и всех его зависимостей.'
        },
        'educational_importance': {
            label: 'Образовательная важность',
            tooltip: 'PageRank только по образовательным связям (PREREQUISITE, ELABORATES, TESTS, EXAMPLE_OF). Показывает важность в контексте обучения.'
        },
        'cluster_id': {
            label: 'Кластер',
            tooltip: 'Идентификатор тематического блока. Узлы одного кластера объединены общей темой или подходом.'
        },
        'bridge_score': {
            label: 'Метрика моста',
            tooltip: 'Композитная метрика узла-моста между тематическими блоками. Высокое значение = узел связывает разные области знаний.'
        }
    },
    ...
    renderEdgesTable(container, node) {
        ...
        if (totalEdges === 0) {
            ...
            ['', 'Связь', 'Тип', 'Узел', 'Вес'].forEach(text => {
    ...
    createEdgeRow(edge, direction, currentNodeId) {
        ...
        typeCell.className = 'edge-type';
        typeCell.textContent = edge.type || 'UNKNOWN';
        ...
        // Node type [Chunk]/[Concept]/[Assessment]
        const nodeTypeCell = document.createElement('td');
        nodeTypeCell.className = 'node-type';
        if (otherNode) {
            ...
            nodeTypeCell.appendChild(badge);
        } else {
            nodeTypeCell.textContent = 'Unknown';
        }
