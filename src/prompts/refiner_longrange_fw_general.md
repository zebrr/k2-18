# Graph Refiner Longrange FORWARD PASS v4.3-gpt-5 @ General (Domain-Agnostic)
...
1. First, check for `PREREQUISITE` ("`Node A` is a prerequisite for `Node B`"):
  ...
  * Use this when `Node A` introduces a fundamental concept that `Node B` is built upon (e.g., `Node A` defines a foundational term or theory, and `Node B` describes an advanced application or specialized case).
...
2. Next, check for other semantic relationships:
  ...
  * `PARALLEL`: `Node A` and `Node B` present alternative approaches or theories for the same problem.
...
- **Node A**: "A system is a set of interrelated elements functioning as a whole. Systems have inputs, outputs, and feedback loops."
- **Node B**: "System dynamics modeling uses stocks, flows, and feedback loops to simulate complex system behavior over time."  
- **Relationship**: A→B, PREREQUISITE, weight=0.85 (must understand basic system concepts before modeling)

Example 2: EXAMPLE_OF
- **Node A**: "The 2008 financial crisis demonstrated how interconnected risks in complex systems can lead to cascading failures across institutions."
- **Node B**: "Systemic risk refers to the potential for failure in one component to trigger failures throughout an interconnected system."
- **Relationship**: A→B, EXAMPLE_OF, weight=0.75 (concrete historical example of systemic risk concept)

Example 3: PARALLEL
- **Node A**: "Top-down analysis begins with the whole system and progressively decomposes it into subsystems and components."
- **Node B**: "Bottom-up analysis starts with individual components and synthesizes them into increasingly complex subsystems."
- **Relationship**: A→B, PARALLEL, weight=0.6 (alternative analytical approaches for the same problem)

Example 4: MENTIONS
- **Node A**: "We will later explore how feedback mechanisms regulate system stability"
- **Node B**: "Feedback loops can be positive (amplifying) or negative (stabilizing), each serving different regulatory functions"
- **Relationship**: A→B, MENTIONS, weight=0.35 (brief reference without elaboration)

Example 5: `"type": null` - No relationship
- **Node A**: "Classification systems organize entities into hierarchical categories based on shared characteristics."
- **Node B**: "Statistical sampling methods determine how to select representative subsets from larger populations."
- **Relationship**: `"type": null` (unrelated methodological topics)
...
```jsonc
{
  "source_node": {
    "id": "course:c:800",
    "text": "Системный анализ рассматривает объект как целостную систему взаимосвязанных элементов. Ключевые принципы: целостность, иерархичность, структурность."
  },
  "candidates": [
    {
    ...
    {
      "node_id": "course:c:2200", 
      "text": "Объектно-ориентированный анализ рассматривает систему как совокупность взаимодействующих объектов с инкапсулированным состоянием и поведением.",
      "similarity": 0.82,
      "existing_edges": [
        {"source": "course:c:800", "target": "course:c:2200", "type": "HINT_FORWARD", "weight": 0.4}
      ]
    },
    {
      "node_id": "course:q:2800:1",
      "text": "Задание: Проведите системный анализ выбранной организации. Определите границы системы, выделите подсистемы, опишите связи между ними.",
      "similarity": 0.75,
      "existing_edges": []
    }
  ]
...
[
  {
    ...
    "type": "PREREQUISITE", // Systems analysis concept is prerequisite for decomposition method
    "weight": 0.9
  },
  {
    "source": "course:c:800",
    "target": "course:c:2200",
    "type": "PARALLEL", // Existing HINT_FORWARD replaced with stronger PARALLEL (both are analytical approaches)
    "weight": 0.65
  },
  {
    "source": "course:c:800",
    "target": "course:q:2800:1",
