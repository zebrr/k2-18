...
class TestSliceProcessor:
    ...
    def test_run_no_slices(self, mock_config, mock_schemas, mock_prompt, tmp_path, monkeypatch):
    ...
    def test_print_start_status(self, mock_config, mock_schemas, mock_prompt, monkeypatch, capsys):
    ...
    def test_update_concept_dictionary_new_concept(self, processor):
        ...
        assert processor.concept_dictionary['concepts'][0]['concept_id'] == "test:p:function"
    ...
    def test_update_concept_dictionary_update_aliases(self, processor):
        ...
        update_concepts = [{
            "concept_id": "test:p:variable",
            "term": {"primary": "Переменная", "aliases": ["var", "variable", "переменная"]},
    ...
    def test_process_chunk_nodes_update_existing(self, processor):
        ...
        assert processor.learning_graph['nodes'][0]['text'] == "Это более длинный текст того же чанка"
    ...
    def test_validate_edges_valid(self, processor):
        ...
        edges = [
            {"source": "chunk1", "target": "chunk2", "type": "PREREQUISITE", "weight": 0.8},
            {"source": "chunk1", "target": "concept1", "type": "MENTIONS", "weight": 0.5}
        ]
    ...
    def test_validate_edges_invalid_reference(self, processor):
        ...
        edges = [
            {"source": "chunk1", "target": "unknown", "type": "PREREQUISITE", "weight": 0.8}
        ]
    ...
    def test_validate_edges_self_loop(self, processor):
        ...
        edges = [
            {"source": "chunk1", "target": "chunk1", "type": "PREREQUISITE", "weight": 0.8},
            {"source": "chunk1", "target": "chunk1", "type": "PARALLEL", "weight": 0.8}  # OK
        ]
    ...
    def test_apply_patch(self, processor):
        ...
        assert edges_added >= 1  # Минимум 1 edge из патча, возможны автоматические MENTIONS
        assert len(processor.concept_dictionary['concepts']) == 1
    ...
    def test_process_single_slice_success(self, processor, sample_slice_file, sample_llm_response, monkeypatch):
    ...
    def test_process_single_slice_with_repair(self, processor, sample_slice_file, sample_llm_response, monkeypatch):
    ...
    def test_process_single_slice_failed_repair(self, processor, sample_slice_file, tmp_path, monkeypatch):
    ...
    def test_full_run_success(self, mock_config, mock_schemas, mock_prompt, tmp_path, monkeypatch, sample_llm_response):
        ...
        with patch('src.itext2kg.OpenAIClient') as mock_client_class:
            ...
            graph_data = json.loads((tmp_path / "out" / "LearningChunkGraph_raw.json").read_text(encoding='utf-8'))
    ...
    def test_add_mentions_edges_basic(self, processor):
        ...
        mentions_edges = [e for e in processor.learning_graph['edges'] if e['type'] == 'MENTIONS']
