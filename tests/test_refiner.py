...
class TestGraphProcessing(unittest.TestCase):
    ...
    def test_empty_target_nodes(self):
        ...
        empty_graph = {
            "nodes": [
                {"id": "node1", "type": "Concept", "text": "Only concepts"}
            ],
            "edges": []
        }
...
class TestCandidateGeneration(unittest.TestCase):
    ...
    def test_generate_candidate_pairs(self):
        ...
        from refiner import generate_candidate_pairs, build_similarity_index
        ...
        source_node = next(n for n in self.nodes if n["id"] == first_pair["source_node"]["id"])
    ...
    def test_generate_candidate_pairs_with_existing_edges(self):
        ...
        index, node_ids = build_similarity_index(embeddings_dict, nodes, self.config, self.logger)
        
        # Добавляем существующее ребро
        edges_index = {
            "node1": {
                "node2": [{"source": "node1", "target": "node2", "type": "PREREQUISITE"}]
            }
        }
        
        candidates = generate_candidate_pairs(
            nodes, embeddings_dict, index, node_ids,
            edges_index, self.config, self.logger
        )
        ...
        if candidates:
            ...
            self.assertEqual(first_candidate["existing_edges"][0]["type"], "PREREQUISITE")


class TestPromptLoading(unittest.TestCase):
    """Тесты загрузки и подготовки промпта."""
    
    @patch('builtins.open', new_callable=mock_open, read_data="Test prompt with {weight_low} and {weight_mid} and {weight_high}")
...
class TestLLMEdgeValidation(unittest.TestCase):
    ...
    def test_validate_valid_edges(self):
        ...
        result = validate_llm_edges(edges_response, "n1", self.candidates, 
    ...
    def test_target_not_in_candidates(self):
        ...
        edges_response = [
            {"source": "n1", "target": "n4", "type": "PREREQUISITE"}  # n4 не в кандидатах
        ]
    ...
    def test_prerequisite_self_loop(self):
        ...
        edges_response = [
            {"source": "n1", "target": "n1", "type": "PREREQUISITE"}
        ]
        
        # Добавляем n1 в кандидаты для теста
        candidates_with_self = self.candidates + [{"node_id": "n1"}]
        
        result = validate_llm_edges(edges_response, "n1", candidates_with_self,
                                  self.graph, self.logger)
    ...
    def test_weight_validation(self):
        ...
        edges_response = [
            {"source": "n1", "target": "n2", "type": "PREREQUISITE", "weight": 1.5},  # > 1
            {"source": "n1", "target": "n3", "type": "ELABORATES", "weight": "invalid"}  # не число
        ]
