...
class TestSlicerLargeFiles:
    ...
    def test_large_file_performance(self, temp_project_dir):
        ...
        try:
            ...
            print(f"Производительность: {throughput_mb_per_sec:.2f} MB/sec ({throughput_mb_per_min:.1f} MB/min)")
            ...
            assert throughput_mb_per_sec > 0.1, f"Производительность {throughput_mb_per_sec:.2f} MB/sec < 0.1 MB/sec"
    ...
    def test_json_vs_md_consistency(self, temp_project_dir):
        ...
        try:
            ...
            md_total_tokens = sum(s['slice_token_end'] - s['slice_token_start'] for s in md_slices.values())
            json_total_tokens = sum(s['slice_token_end'] - s['slice_token_start'] for s in json_slices.values())
            ...
            if md_total_tokens > 0 and json_total_tokens > 0:
                token_diff_percent = abs(md_total_tokens - json_total_tokens) / max(md_total_tokens, json_total_tokens) * 100
                print(f"  Разница в токенах: {token_diff_percent:.1f}%")
                
                assert token_diff_percent < 20, \
            ...
            slice_diff_percent = abs(len(md_slices) - len(json_slices)) / max(len(md_slices), len(json_slices)) * 100
            print(f"  Разница в количестве слайсов: {slice_diff_percent:.1f}%")
            
            assert slice_diff_percent < 20, \
        ...
        try:
            ...
            for i, slice_file in enumerate(slice_files):
                try:
                    ...
                    if start >= end:
                        validation_errors.append(f"{slice_file.name}: start >= end ({start} >= {end})")
                    
                    # Упрощенная проверка размера - просто проверяем что не превышает max_tokens
                    if end - start > test_config['slicer']['max_tokens']:
                        validation_errors.append(f"{slice_file.name}: slice too large ({end - start} tokens)")
            ...
            assert len(validation_errors) == 0, f"Найдены ошибки валидации: {len(validation_errors)}"
    ...
    def test_deterministic_large_file(self, temp_project_dir):
        ...
        try:
            ...
            assert first_run_slices.keys() == second_run_slices.keys(), \
            ...
            for slice_id in first_run_slices:
                ...
                for field in ['order', 'source_file', 'slug', 'slice_token_start', 'slice_token_end']:
            ...
            assert len(differences) == 0, f"Результаты не детерминированы: {len(differences)} различий"
            
            print(f"✅ Детерминированность подтверждена для {len(first_run_slices)} слайсов")
