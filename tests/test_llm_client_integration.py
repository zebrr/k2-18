...
from src.utils.llm_client import OpenAIClient, TPMBucket, ResponseUsage, IncompleteResponseError
...
def integration_config(api_key):
    ...
    if config['itext2kg'].get('model_test', '').startswith('o'):
        test_config['reasoning_effort'] = config['itext2kg'].get('reasoning_effort', 'medium')
        test_config['reasoning_summary'] = config['itext2kg'].get('reasoning_summary', 'auto')
...
class TestOpenAIClientIntegration:
    ...
    def test_simple_response(self, integration_config):
        ...
        # Проверка что bucket обновился - может остаться initial_limit если реальный лимит выше
        assert client.tpm_bucket.remaining_tokens <= client.tpm_bucket.initial_limit
        # УДАЛИТЬ эту проверку - в async режиме reset_time обновляется через probe
    ...
    def test_response_chain(self, integration_config):
        ...
        text2, id2, usage2 = client.create_response(
            instructions="Continue being a math tutor.",
    ...
    def test_reasoning_model(self, integration_config):
        ...
        if not integration_config['model'].startswith('o'):
            pytest.skip(f"Test requires reasoning model")
    ...
    def test_headers_update(self, integration_config):
        ...
        assert client.tpm_bucket.remaining_tokens <= initial_remaining
    ...
    def test_background_mode_verification(self, integration_config, capsys):
        ...
        response_text, response_id, usage = client.create_response(
            ...
            input_data="Say 'background test'"
        )
    ...
    def test_console_progress_output(self, integration_config, capsys):
        ...
        assert has_queue or has_progress, f"Expected QUEUE or PROGRESS messages, but got neither. Captured: {captured.out[:200]}"
