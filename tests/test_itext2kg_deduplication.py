    ...
    def test_process_mixed_nodes(self):
        ...
        new_nodes = [
            # Дубликат Chunk с длинным текстом
            {
                "id": "test:c:100",
                "type": "Chunk",
                "text": "Обновленный более длинный чанк",
                "local_start": 100,
            },
            # Новый Chunk
            {
...
class TestIncrementalValidation(unittest.TestCase):
    ...
    def test_validation_error_handling(self, mock_graph_val):
        ...
        mock_graph_val.side_effect = GraphInvariantError(
            "Дублированный ID узла (Chunk): test:c:100"
        )
...
class TestEdgeDeduplication(unittest.TestCase):
    ...
    def test_validate_edges_filters_duplicates(self):
        ...
        # Проверяем результаты
        self.assertEqual(
            len(valid_edges), 2, f"Expected 2 valid edges, got {len(valid_edges)}"
        )
        ...
        with patch.object(self.processor.logger, "info") as mock_info:
            ...
            for call in mock_info.call_args_list:
                ...
                try:
                    ...
                    if (
                        log_data.get("event") == "edge_dropped"
                        and log_data.get("reason") == "duplicate_edge"
                    ):
                        log_call_found = True
            ...
            self.assertTrue(
                log_call_found, "Expected duplicate edge log message not found"
            )
